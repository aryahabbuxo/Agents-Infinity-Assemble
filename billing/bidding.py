...
OUT_OF_DOMAIN_PENALTIES = {
    "locked": 0.50,
    "unlock": 0.50,
    "login": 0.40,
    "log in": 0.40,
    "password": 0.40,
    "cancel my subscription": 0.45,
    "cancel immediately": 0.45,
    "want to cancel": 0.40,
}
...
def _rule_based_confidence(ticket_text: str) -> float:
    ...
    for kw, weight in BILLING_KEYWORDS.items():
        if kw in text:
            score += weight
    for kw, p in OUT_OF_DOMAIN_PENALTIES.items():
        if kw in text:
            score -= p
...
def estimate_raw_confidence(ticket_text: str) -> float:
    ...
    """
    text = ticket_text.lower()
    penalty = sum(p for kw, p in OUT_OF_DOMAIN_PENALTIES.items() if kw in text)
    ...
    try:
        ...
        if value > 1.0:  # model gave e.g. "85" meaning 0.85
            value = value / 100.0
        val = max(0.0, min(value - penalty, 0.97))
        return val
