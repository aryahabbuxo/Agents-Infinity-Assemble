...
class RemoteBillingAgent:
    ...
    def respond_to_proposal(self, proposal):
        ...
        try:
            ...
            data = response.json()
            action = data.get("action") or data.get("message_type")
            if action not in {
                "PROPOSE",
                "COUNTER",
                "ACCEPT",
                ...
                raise ValueError(
                    f"Invalid negotiation action: {action}"
                )
            ...
            return {
                ...
                "action": "ACCEPT",
                "message_type": "ACCEPT",
                "rationale": f"Billing offline fallback: accepted by mock ({exc})",
            }
...
class RetentionAgent:
    ...
    def submit_bid(self, ticket):
        ...
        calibrated_confidence = self.instance_index.get_adjusted_confidence(raw_confidence, ticket_text)
    ...
    def produce_final_statement(
        ...
        if "cancel_subscription" in successful_actions:
            ...
            return self._append_collaboration_summary(
    ...
    def _append_collaboration_summary(self, base_statement, collaboration_result):
        """Append a billing/technical collaboration summary to a base statement."""
        if not collaboration_result:
            return base_statement
        collab_stmt = collaboration_result.get("statement") or collaboration_result.get("result")
        if collab_stmt:
            return f"{base_statement} Additionally (via billing collaboration): {collab_stmt}"
        return base_statement
    ...
    def respond_to_proposal(self, proposal):
        """Respond to a proposal received from a peer agent."""
        sender = proposal.get("sender", "peer_agent")
        action = proposal.get("action") or proposal.get("message_type")

        if action not in {"PROPOSE", "COUNTER"}:
            return {
                "sender": self.agent_id,
                "receiver": sender,
                "action": "ACCEPT",
                "message_type": "ACCEPT",
                "rationale": "Acknowledgement received.",
            }

        requested = (proposal.get("your_requested_subtask") or proposal.get("requested_subtask") or "").lower()
        my_task = (proposal.get("my_assigned_subtask") or proposal.get("my_subtask") or "").lower()
        combined = f"{requested} {my_task}"

        retention_terms = ["retention", "cancel", "cancellation", "churn", "dissatisfied", "frustrated", "offer", "loyalty", "customer"]
        is_retention = any(term in combined for term in retention_terms)

        if is_retention or not combined.strip():
            ...
            return {
                ...
                "action": "ACCEPT",
                "message_type": "ACCEPT",
                "rationale": "Retention Agent accepts the requested subtask.",
            }
        ...
        return {
            ...
            "action": "REJECT",
            "message_type": "REJECT",
            "rationale": "The requested subtask is outside Retention Agent capabilities.",
        }
    def execute_collaboration(
        ...
        if hasattr(counterpart, "handle_collaboration"):
            result = counterpart.handle_collaboration(
                ticket=ticket,
                order_id=order_id,
                subtask=subtask,
            )
        elif hasattr(counterpart, "collaborate_url"):
            try:
                response = requests.post(
            ...
            result = {
                "success": False,
                "error": "Counterpart agent does not support collaboration interface.",
            }
    ...
    def handle_ticket(
        ...
        ticket_id = ticket["ticket_id"]
        ticket_text = ticket["text"]
        self.current_ticket_text = ticket_text
        self.sandbox.current_ticket_id = ticket_id
...
"""
    ...
    def ask_llm(self, user_message):
        ...
        try:
            ...
            print(f"[{self.agent_id}] LLM unavailable ({exc}). Using fallback retention plan decision.")
            ticket_txt = getattr(self, "current_ticket_text", "").lower()
            cancellation_in_ticket = any(k in ticket_txt for k in ["cancel my subscription", "cancel immediately", "want to cancel", "cancel subscription"])
            # Rule-based fallback decision: check explicit cancellation request in ticket text AND sandbox state
            if ("cancellation_requested\": true" in user_message.lower() or "cancellation_requested=true" in user_message.lower()) and cancellation_in_ticket:
                fallback = '{"actions": ["cancel_subscription"]}'
            elif "retention_offer_available\": true" in user_message.lower() or "retention_offer_available=true" in user_message.lower():
                fallback = '{"actions": ["apply_retention_offer"]}'
            else:
                fallback = '{"actions": []}'
            return fallback
